...
import { ElementType } from '../editorRendererController/editorState'
import { Button, Flex } from '../../components'
import { mdiBorderInside, mdiBorderOutside, mdiSort } from '@mdi/js'
import { EditorStateType } from '../editorRendererController'

export type RootElementOverlayProps<ControllreActionsType> = {
  element: ElementType
...
>(
  props: RootElementOverlayProps<ControllreActionsType>
) => {
  const { element, editorState, actions, isProduction } = props
  const { selectElement, selectHoveredElement } = actions?.ui ?? {}
  ...
  useEffect(() => {
    ...
    const onHover = (e: any) => {
      // e.stopPropagation()
      selectHoveredElement?.(elementId, true)
      // setOpenMenu(true)
    }
    const onMouseOut = (e: any) => {
      e.stopPropagation()

      selectHoveredElement?.(elementId, false)
      // setOpenMenu(false)
    }
  ...
  }, [selectHoveredElement, elementId])
  ...
  const handleClickOverlay = useCallback(
    (e: MouseEvent) => {
      ...
      selectElement?.(elementId, serializedBoundingRect)
    },
    [selectElement, elementId, handleToggleMenu]
  )

  const handleStartDragging = useCallback(
    (e: MouseEvent) => {
      e.stopPropagation()
      if (!actions?.ui) return
