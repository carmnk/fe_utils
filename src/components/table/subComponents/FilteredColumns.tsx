...
export const FilteredTableHeaderCell = (
  props: FilteredTableHeaderCellProps
) => {
  const {
    onOpen,
    onClose,
  ...
  useImperativeHandle(ref, () => anchor?.current as HTMLTableCellElement)
  ...
  const handleKeyUpMenuItem = useCallback(
    (e: KeyboardEvent, item: GenericOptionsType) => {
      if (e?.type !== 'keyup' || e?.key !== 'Enter' || filterKey === undefined)
        return
